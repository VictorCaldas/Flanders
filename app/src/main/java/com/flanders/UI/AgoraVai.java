...
public class AgoraVai extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.agoravai_activity);
    ...
    private void discover() {
        ...
        Nearby.Connections.startDiscovery(googleApiClient, serviceId, CONNECTION_TIME_OUT, this)
                .setResultCallback(new ResultCallback<Status>() {
                    ...
                    public void onResult(Status status) {
                        if (status.isSuccess()) {
                            AgoraVai.this.status.setText("Discovering");
                        }
                    }
                });
    }
        ...
        Nearby.Connections.sendConnectionRequest(googleApiClient, deviceId, endpointId, payload,
                new Connections.ConnectionResponseCallback() {
                    ...
                    public void onConnectionResponse(String s, Status status, byte[] bytes) {
                        if (status.isSuccess()) {
                            AgoraVai.this.status.setText("Connected to: " + s);
                            ...
                            AgoraVai.this.status.setText("Connection to " + s + " failed");
                            if (!isHost) {
                                isConnected = false;
                            }
                        }
